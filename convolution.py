...
def multiple_convs_kan_conv2d(matrix, #but as torch tensors. Kernel side asume q el kernel es cuadrado
    ...
    # print('222222222222222222', matrix.shape, conv_groups.shape, matrix_out.shape)
    ...
    # print('222222222222222222', unfold(matrix[:,:,:,:]).shape)
    for kern in range(n_convs):
        matrix_out[:, kern, :, :] = kernels[kern].conv.forward(
            conv_groups[:, :, :].flatten(0, 1)).reshape((batch_size, h_out, w_out))

    # for channel in range(n_channels):
    #     for kern in range(n_convs):
    #         matrix_out[:,kern  + channel*n_convs,:,:] = kernels[kern].conv.forward(conv_groups[:,channel,:,:].flatten(0,1)).reshape((batch_size,h_out,w_out))
    # print('222222222222222222', matrix_out.shape)
    return matrix_out
